...
class VariableImpedanceControllerDOB: public controller_interface::Controller<hardware_interface::EffortJointInterface>
{
public:
  ~VariableImpedanceControllerDOB() override
  {
  ...
  {
    ROS_INFO("Initialize Variable Impedance Controller with DOB");
    ...
    for (size_t i=0; i<n_joints_; i++)
    {
      ...
      //Kp
      if ( n.getParam("/singlearm/variable_impedance_controller_dob/joint" + si + "/tdc/p", Kp[i]) )
      {
        Kp_(i) = Kp[i];
      }
      else
      ...
      //Kd
      if ( n.getParam("/singlearm/variable_impedance_controller_dob/joint" + si + "/tdc/d", Kd[i]) )
      {
        Kd_(i) = Kd[i];
      }
      else
    ...
    if (!n.getParam("/singlearm/variable_impedance_controller_dob/mode", experiment_mode_))
    {
      ROS_ERROR("Cannot find mode");
      return false;
    }

    if (!n.getParam("/singlearm/variable_impedance_controller_dob/aic/alpha", alpha))
    {
      ROS_ERROR("Cannot find alpha");
      return false;
    }

    if (!n.getParam("/singlearm/variable_impedance_controller_dob/aic/epsilon", epsilon))
    {
      ROS_ERROR("Cannot find alpha");
      return false;
    }
    ...
    sub_command_ = n.subscribe<std_msgs::Float64MultiArray>("command",1, &VariableImpedanceControllerDOB::commandCB,this);
    sub_forcetorque_sensor_ = n.subscribe<geometry_msgs::WrenchStamped>("/singlearm/ft_sensor_topic", 1, &VariableImpedanceControllerDOB::updateFTsensor, this);
  ...
  void starting(const ros::Time& time) override
  {
    ...
    ROS_INFO("Starting Variable Impedance Controller with DOB");
  }
...
PLUGINLIB_EXPORT_CLASS(singlearm_controller::VariableImpedanceControllerDOB, controller_interface::ControllerBase)
